...
namespace OpenUtau.Core.DiffSinger {
    ...
    public abstract class DiffSingerRhythmizerBasePhonemizer : MachineLearningPhonemizer {
        ...
        public virtual void SetUpPhoneDictAndRhy() {
            LoadRhythmizer();
        }

        // JPN,dsdur目录名
        public void LoadSingerRhythmizer(string lang = "CNM") {
            var diyRhyDict = new Dictionary<string, string>();
            string path = Path.Combine(singer.Location, "diy_rhy.txt");
            if (File.Exists(path)) {
                foreach (string line in File.ReadLines(path, System.Text.Encoding.UTF8)) {
                    string[] elements = line.Split(",");
                    try {
                        diyRhyDict.Add(elements[0].Trim(), elements[1].Trim());
                    } catch (Exception ex) {
                        Log.Information(elements[0].Trim() + "的rhy名字有问题");
                        continue;
                    }
                }
            } else {
        ...
        public void LoadRhythmizer(string baseRhyName = "") {
            ...
            try {
                ...
                Log.Information("加载Rhythmizer失败");
                return;
            }
        }
        ...
        public void GetRealPhnDict(string dictFile) {
            try {
                string path = Path.Combine(singer.Location, dictFile);
